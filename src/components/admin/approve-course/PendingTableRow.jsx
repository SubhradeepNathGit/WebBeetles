    ...
    return (
        <tr className="hover:bg-white/[0.02] transition-colors group text-center">
            ...
            <td className="px-6 py-4 text-sm text-gray-400 font-medium">
                <div className="flex items-center gap-1.5"><User size={13} className="text-emerald-400" />{course?.instructor?.name ?? 'N/A'}</div>
            </td>
            <td className="px-6 py-4">
                <span className="text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-md font-medium">{course?.category?.name ?? 'N/A'}</span>
            </td>
            ...
            <td className="px-6 py-4 text-right">
                <div className="flex items-center justify-end gap-2">
                    <button onClick={() => setPreview(course)} className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 rounded-lg transition-colors text-xs font-semibold cursor-pointer">
                        <Eye size={13} /> Preview
                    </button>
