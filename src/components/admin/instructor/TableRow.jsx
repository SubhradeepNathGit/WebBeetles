...
const TableRow = ({ inst,setOpenMarkModal, setInstructorId, setChangeStatus }) => {
    ...
    const { isLoading: isCourseLoading, data: courseData } = useInstructorCourses(inst?.id);
    const { isLoading: isStatsLoading, data: statusData } = useInstructorStats(inst?.id);
        ...
        <>
            <tr className="hover:bg-white/[0.02] transition-colors group text-center">
                ...
                <td className="px-6 py-4 text-sm text-gray-300 font-medium">
                    <div className="flex items-center gap-1.5"><BookOpen size={13} className="text-emerald-400" />{inst?.application_status === "approved" ? (isCourseLoading ? <span className="inline-block w-4 h-4 rounded bg-white/5 animate-pulse"></span> : courseData?.length ?? 'N/A') : '0'}</div>
                </td>
                <td className="px-6 py-4 text-sm text-gray-300 font-medium">{inst?.application_status === "approved" ? (isStatsLoading ? <span className="inline-block w-4 h-4 rounded bg-white/5 animate-pulse"></span> : statusData?.totalStudents ?? 'N/A') : '0'}</td>
                <td className="px-6 py-4 text-sm text-yellow-500 font-semibold">{"₹" + (inst?.application_status === "approved" ? (isStatsLoading ? <span className="inline-block w-12 h-4 rounded bg-white/5 animate-pulse"></span> : statusData?.totalRevenue?.toLocaleString() ?? 'N/A') : '0')}</td>
                <td className="px-6 py-4">
