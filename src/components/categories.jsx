...
const CategoriesSection = () => {
  // Animation variants for rows (each row animates as a unit)
  const rowVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
  ...
  // Function to get card groups for animation
  const getCardGroups = () => {
    const groups = [];
    ...
    <section className="bg-black text-white py-16 px-4 lg:py-20">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <motion.div variants={headerVariants} className="lg:max-w-md lg:text-right">
        ...
        >
          {getCardGroups().map((group, groupIndex) => (
            <motion.div
              ...
              className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-12 gap-6"
            >
              {group.map((cat, index) => {
             ...
             group-hover:bg-white/10 group-hover:backdrop-blur-md group-hover:border group-hover:border-white/20"
                    ...
                    {/* Content */}
                    <div className="pt-4">
                      <h3 className="text-xl lg:text-2xl font-bold mb-3 group-hover:text-white transition-colors duration-300">{cat.title}</h3>
        ...
        {/* Browse All Button - Bottom Right */}
        <motion.div
        ...
        >
          <button className="bg-purple-700 hover:bg-purple-600 transition-all duration-300 px-6 py-3 rounded-full 
