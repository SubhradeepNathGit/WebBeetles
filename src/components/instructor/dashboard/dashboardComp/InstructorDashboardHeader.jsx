...
const InstructorDashboardHeader = ({ instructorDetails }) => {
    ...
    return (
        <div className="bg-gradient-to-r from-rose-500/30 via-red-500/30 to-pink-500/30 backdrop-blur-xl rounded-2xl lg:rounded-3xl shadow-2xl border border-white/20 p-4 sm:p-5 lg:p-8">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-4 lg:gap-6">
                <div className="relative flex-shrink-0">
                    <div className="w-20 h-20 sm:w-24 sm:h-24 lg:w-28 lg:h-28 rounded-full ring-4 ring-white/30 overflow-hidden shadow-2xl bg-gradient-to-br from-rose-400 to-red-500">
                ...
                <div className="flex-1 text-center md:text-left w-full">
                    <div className="flex items-center justify-center md:justify-start gap-2 mb-2 lg:mb-3 flex-wrap">
                        <span className="text-xs font-bold text-rose-200 bg-white/20 px-2.5 py-1 rounded-full">INSTRUCTOR DASHBOARD</span>
                    ...
                    <h1 className="text-xl sm:text-2xl lg:text-3xl xl:text-4xl font-bold text-white mb-2 lg:mb-3">
                        Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-300 to-red-300">{name?.split(" ")[0]}</span>! 🎓
                    </h1>

                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-3 lg:mb-4 text-xs sm:text-sm">
                        {email && <div className="flex items-center gap-1.5 text-rose-200 bg-white/10 px-3 py-1.5 rounded-lg border border-white/20 max-w-full"><Mail size={14} className="flex-shrink-0" /><span className="truncate">{email}</span></div>}
                        {slug && <div className="flex items-center gap-1.5 text-rose-200 bg-white/10 px-3 py-1.5 rounded-lg border border-white/20 max-w-full"><UserCircle size={14} className="flex-shrink-0" /><span className="truncate">@{slug}</span></div>}
                        {createdAt && <div className="flex items-center gap-1.5 text-rose-200 bg-white/10 px-3 py-1.5 rounded-lg border border-white/20 max-w-full"><Calendar size={14} className="flex-shrink-0" /><span className="truncate">Joined {formatDate(createdAt)}</span></div>}
                    </div>
                    ...
                    {!editingBio ? (
                        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-2 lg:gap-3">
                            <div className="flex-1 bg-white/10 px-3 sm:px-4 py-2.5 sm:py-3 lg:py-4 rounded-xl border border-white/20">
                                <p className="text-rose-100 text-xs sm:text-sm lg:text-base">{bio || "No bio added yet."}</p>
                            </div>
                            <button onClick={handleEditBio} className="inline-flex items-center justify-center gap-2 text-xs sm:text-sm font-semibold text-white bg-rose-600/50 hover:bg-rose-600/70 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-rose-400/40 transition-all hover:shadow-lg active:scale-95 cursor-pointer">
                        ...
                        <div className="flex flex-col gap-2 lg:gap-3">
                            <div className="relative">
                                <textarea value={tempBio} onChange={(e) => setTempBio(e.target.value)} rows={3} maxLength={500} placeholder="Write a short bio about yourself..." className="bg-white/10 text-rose-100 placeholder:text-rose-300/50 rounded-xl w-full text-xs sm:text-sm p-3 border border-white/20 focus:outline-none focus:ring-2 focus:ring-rose-400/50 resize-none" />
                                <span className="absolute bottom-2 right-2 text-xs text-rose-300/70 bg-black/20 px-2 py-1 rounded-lg">{tempBio.length}/500</span>
                            </div>
