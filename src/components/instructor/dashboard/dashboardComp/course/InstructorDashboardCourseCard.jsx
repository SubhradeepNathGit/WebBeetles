        ...
        <div className="group bg-white/5 rounded-xl p-3 sm:p-4 hover:bg-white/15 transition-all border border-white/10 hover:border-white/30 hover:shadow-2xl">
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <div className="relative flex-shrink-0">
                    <img src={course?.thumbnail} alt={course?.title} className="w-full sm:w-32 lg:w-40 aspect-video sm:aspect-auto sm:h-24 lg:h-28 rounded-lg object-cover ring-2 ring-white/20 group-hover:ring-white/40 transition-all" />
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3">
                        <span className="inline-flex items-center gap-1 bg-blue-500/20 px-2 py-1 rounded-md text-xs text-rose-200 font-medium border border-white/10"><Users size={12} />{students?.length?.toLocaleString() ?? 0}</span>
                        <span className="inline-flex items-center gap-1 bg-green-500/20 px-2 py-1 rounded-md text-xs text-rose-200 font-medium border border-white/10"><IndianRupee size={12} />{course?.price?.toLocaleString() ?? 0}</span>
                        <span className="inline-flex items-center gap-1 bg-yellow-500/20 px-2 py-1 rounded-md text-xs text-rose-200 font-medium border border-white/10"><Star size={12} className="text-yellow-400 fill-yellow-400" /><CourseRating courseId={course?.id} /></span>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-white/10 rounded-lg p-2 sm:p-2.5 border border-white/20">
                            <div className="flex items-center gap-1.5 mb-1"><Eye size={12} className="text-blue-400" /><span className="text-xs text-rose-200 font-medium">Views</span></div>
                            <p className="text-base sm:text-lg font-bold text-white">{totalViews?.toLocaleString()}</p>
                        </div>
                        <div className="bg-white/10 rounded-lg p-2 sm:p-2.5 border border-white/20">
                            <div className="flex items-center gap-1.5 mb-1"><Target size={12} className="text-green-400" /><span className="text-xs text-rose-200 font-medium">Lecture</span></div>
